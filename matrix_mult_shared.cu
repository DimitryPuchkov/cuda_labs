...
int main()
{
	...
	float toDeviceTime;
	cudaEventElapsedTime(&toDeviceTime, start, stop);
	printf("Copy to Device time: %.2f milliseconds\n", toDeviceTime);
	...
	cudaEventSynchronize(stop);
	float toHostTime;
	cudaEventElapsedTime(&toHostTime, start, stop);
	printf("Copy to Host time: %.2f milliseconds\n", toHostTime);
